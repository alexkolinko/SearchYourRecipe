...
struct Constant {
    ...
    struct URLs {
        static func recipesByName(_ name: String) -> URL {
            return URL(string: "\(Constant.BASE_URL)?type=public&beta=false&q=\(name)&app_id=\(Constant.APP_ID)&app_key=\(Constant.API_KEY)")!
        }
        
        static func recipeById(_ id: String) -> URL {
            return URL(string: "\(Constant.BASE_URL)/\(id)?type=public&app_id=\(Constant.APP_ID)&app_key=\(Constant.API_KEY)")!
        }
